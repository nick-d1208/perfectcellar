...
var quizzesView = (function(){
	...
	if(ClickControl){
		_clickControls["back"] = new ClickControl({selector:'#quizzes',childSelector:'.mp3-header .icon-left',callback:function(e){
			settings.backButtonClicked.notify();
		}});
		_clickControls["answer"] = new ClickControl({selector:'#quizzes',childSelector:'.quiz-item .answer-item',callback:function(e){
			$(this).addClass('active').siblings().removeClass('active');
			settings.quizAnswerClicked.notify({quizid:$(this).attr('data-quiz'), id:$(this).attr('data-id')});
		}});
		_clickControls["home"] = new ClickControl({selector:'#quizzes',childSelector:'.mp3-header .icon-right',callback:function(e){
			settings.homeButtonClicked.notify();
		}});
		_clickControls["share"] = new ClickControl({selector:'#quizzes',childSelector:'.share .sharebox',callback:function(e){
			settings.shareButtonClicked.notify();
		}});
		_clickControls["previousquizbutton"] = new ClickControl({selector:'#quizzes',childSelector:'.previous-container .previous-button',callback:function(e){
			settings.previousQuizButtonClicked.notify();
		}});
		_clickControls["previousquizitem"] = new ClickControl({selector:'#quizzes',childSelector:'.previous-container .previous-item',callback:function(e){
	...
	settings.populateTemplate = function(templateData){
		...
		_iosScrollView = new IScroll('#quizzes .wrapper-scroller',{hScrollbar:false, vScrollbar:false,hideScrollbar: true, hideScrollbars: true});
	};	
